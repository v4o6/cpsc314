...
// What myPolygonMode() to call in scenarios. Not static so it can be used in scenarios.cpp
bool drawPolysAsPoints = false;
...
void reshape(int w, int h) {
    ...
    // Setup the emulated GL environment inside the real OpenGL one
    double virtualWidth = realWidth / virtualPixelSize;
    double virtualHeight = realHeight / virtualPixelSize;
    glViewport(0, 0, virtualWidth * virtualPixelSize, virtualHeight * virtualPixelSize);

    // Call the virtual viewport method.
...
static void keyboard(unsigned char key, int x, int y) {
    ...
    switch(key) {
        ...
        case 't':
            ...
            break;
        case 'p':
            drawPolysAsPoints = !drawPolysAsPoints;
            break;
        case 'o':
